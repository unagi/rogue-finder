...
6. **詳細アクションを実行** — 結果一覧で対象ホストを選ぶと、**高精度実行** / **高精度 + OS 判定** / **全ポート実行** / **全ポート + OS 判定** / **安全診断実行** が使えます。全ポート実行は `scan.advanced_timeout_seconds` と `scan.advanced_max_parallel` を使うため、通常の高精度実行より長くなることがあります。
7. **証跡をエクスポート** — **Export CSV** / **Export JSON** で UTF-8 ファイルを出力します。構造化された `errors` とローカライズ済みの `errors_text` の両方が含まれます。
