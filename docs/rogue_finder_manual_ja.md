...
- Windows 版では GUI 本体は標準ユーザー権限のまま、UAC で 1 度だけ昇格したランナープロセスが `nmap` を実行し、Named Pipe 経由でログ/進捗/結果を転送します。
...
## 設定ファイル
起動時に現在のディレクトリで `rogue-finder.config.yaml` を探し、無ければ安全な既定値で生成します。バージョンアップ時は不足キーのみ自動追加され、既存の上書きは保持されます。この YAML で `scan`（タイムアウトやポートリスト）、`rating`（加点ルールや優先度境界）、`safe_scan`（セーフスクリプト診断の ETA 推定）、`ui`（行カラー）、`runtime`（Windows ランナー動作）を調整できます。`runtime.windows_privileged_runner` を `false` にすると、従来通り単一プロセスで起動します。PyInstaller 配布物に組織固有のデフォルトを同梱したい場合は、このファイルを一緒に配布してください。
...
- **Windows の UAC 制御:** GUI 起動後に初めてスキャンを開始すると、UAC ダイアログを 1 度だけ表示して特権ランナーを生成します。同じセッション中はランナーを再利用するため、以降のスキャンでは追加の確認はありません。ポリシー上 UAC 提示が許可されない場合は `runtime.windows_privileged_runner: false` に設定し、GUI 全体を管理者権限で実行してください。
